**로직 생성기 가이드**
 - 자동 매매 로직 생성기 가이드
    ...
    - 각 설정 항목별 설정가이드(comment) 작성.(오른쪽에 Comment 컴포넌트에서 상세 설명)
...
**설정(Setting)**
 -  로직 실행 기본 세팅 설정(CRUD 기본)
    1. 로직명 입력: str
        - 자동 매매 로직 이름을 입력,저장,불러오기,수정,삭제 할 수 있도록 함.(로직명 중복확인, 중복불가)
    2. 기본 세팅
 ...
 - 매도 조건 설정(아래 조건중 선택 하나만 만족하거나, 2개이상 만족하거나 선택가능, 체크박스로 입력 및 수정, 선택하지 않은 조건 비활성)
    ...
    2. 포지션 포기 조건(손절)
        ...
        - Stoploss 라인 생성 선택
            ...
            · Stoploss 라인 가격 기준
                - 수익율 기준: - 현재 수익율 : 1000% ~ -1000% 입력값 선택
                - 지표 사용
                    · 캔들값은 Open,High,Close,Low 기준값 중 선택
                    · 볼린저밴드 값은 상단, 중간, 하단 기준값 중 선택 (지표 Input 설정, 기본은 20,2,0)
                    · MA 값은 1~500 Float 으로 입력값 받음
...
**로직 설정 저장**
 - 사용자가 설정한 로직은 DB에 따로 저장 및 백업. 시스템 관리자 페이지에서 확인 가능.
 - 로직 생성 로그 저장.
 - 선택 항목 중 미입력 항목 있을 경우 아직 입력하지 않은 정보 확인 메세지. 임시저장 기능.
